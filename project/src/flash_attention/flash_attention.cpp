...
utils::AttentionResult
launch_flash_attention_kernels(float *Q_ptr, float *K_ptr, float *V_ptr,
                               float *O_ptr, int seq_len, int head_dim);
...
// calls kernel with params as in the paper
torch::Tensor forward(torch::Tensor Q, torch::Tensor K, torch::Tensor V) {
  ...
  TORCH_CHECK(result.cudaError == cudaSuccess, "CUDA kernel launch failed: ",
              cudaGetErrorString(result.cudaError));

  return O;
}

// call with custom params
torch::Tensor forward_with_parameters(torch::Tensor Q, torch::Tensor K,
                                      torch::Tensor V, int B_c, int B_r,
  ...
  // does not need to be initialized to zero, kernel does that
  torch::Tensor O = torch::empty_like(Q);
...
PYBIND11_MODULE(TORCH_EXTENSION_NAME, m) {
  ...
  m.def("forward_with_parameters", &flash_attention::forward_with_parameters,
